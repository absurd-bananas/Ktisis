...
public class GPoseService : IDisposable
{
  ...
  {
    add => this._gposeEvent.Add(new Action<GPoseService, bool>(value.Invoke));
    remove => this._gposeEvent.Remove(new Action<GPoseService, bool>(value.Invoke));
  }
