...
public sealed class DalamudServices
{
  ...
  public void Add(IDalamudPluginInterface dpi, IServiceCollection services)
  {
    ...
    services.AddSingleton<IDalamudPluginInterface>(dpi).AddSingleton<IUiBuilder>(dpi.UiBuilder).AddSingleton<IClientState>(this.ClientState).AddSingleton<ICommandManager>(this.Cmd).AddSingleton<IFramework>(this.Framework).AddSingleton<IGameGui>(this.Gui).AddSingleton<IGameInteropProvider>(this.Interop).AddSingleton<IObjectTable>(this.ObjectTable).AddSingleton<IKeyState>(this.KeyState).AddSingleton<IDataManager>(this.Data).AddSingleton<ITextureProvider>(this.Tex).AddSingleton<ISigScanner>(this.SigScanner).AddSingleton<IChatGui>(this.ChatGui).AddSingleton<ITargetManager>(this.TargetManager);
  }
}
