...
public class LocaleData
{
  ...
  public string Translate(string key, Dictionary<string, string>? parameters = null)
  {
    ...
    if (this.warnedKeys.Add(key))
      ...
      {
        (object) key,
        (object) this.MetaData.TechnicalName
      });
    return key;
  }
  ...
  {
    StringBuilder stringBuilder1 = new StringBuilder(translationString.Length);
    StringBuilder stringBuilder2 = new StringBuilder(16 /*0x10*/);
    bool flag = false;
    foreach (char ch in translationString)
    {
      if (!flag)
      {
        if (ch == '%')
