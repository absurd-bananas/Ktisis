...
public class JsonFileSerializer {
	...
	public JsonConverter<T> GetConverter<T>() => (JsonConverter<T>)this.Options.GetConverter(typeof(T));

	public string Serialize(object obj) => JsonSerializer.Serialize<object>(obj, this.Options);

	public T? Deserialize<T>(string json) where T : notnull => JsonSerializer.Deserialize<T>(json, this.DeserializeOptions);
}
