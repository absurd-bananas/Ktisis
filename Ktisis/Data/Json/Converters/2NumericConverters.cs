...
internal class Vector4Converter : JsonConverter<Vector4> {
	...
	public virtual Vector4 Read(ref Utf8JsonReader reader, Type type, JsonSerializerOptions options) {
		...
		return new Vector4(float.Parse(strArray[0], CultureInfo.InvariantCulture), float.Parse(strArray[1], CultureInfo.InvariantCulture), float.Parse(strArray[2], CultureInfo.InvariantCulture),
