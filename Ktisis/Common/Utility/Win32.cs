...
public static class Win32 {
	...
	public unsafe static (Point pos, Size size, int style, Size deviceSize) GetWinProperties() {
		...
		RECT lpRect;
		GetWindowRect(GameWindowHandle, out lpRect);
		var windowLong = GetWindowLong(GameWindowHandle, -16);
	...
	public unsafe static void SetWinDefault(PyonConfig cfg) {
		if (cfg.DefaultSize == Size.Empty)
			return;
		(Point pos, Size size, var style, Size deviceSize) = GetWinProperties();
	...
	public unsafe static void SetWinRes(PyonConfig cfg) {
		...
		if ((int)devicePtr->Width != cfg.HiResSize.Width || (int)devicePtr->Height != cfg.HiResSize.Height) {
			SetWindowLong(GameWindowHandle, -16, cfg.DefaultStyle & 16777216 /*0x01000000*/);
			ShowWindow(GameWindowHandle, 3);
	...
	public struct RECT(int left, int top, int right, int bottom) {
		...
		public bool Equals(RECT r) => r.Left == this.Left && r.Top == this.Top && r.Right == this.Right && r.Bottom == this.Bottom;
		...
		public override bool Equals(object obj) {
			switch (obj) {
				...
				case Rectangle r2:
					return this.Equals(new RECT(r2));
				default:
					return false;
			}
		}
