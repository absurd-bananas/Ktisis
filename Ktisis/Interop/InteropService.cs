...
public class InteropService : IDisposable
{
  ...
  {
    InteropService.HookMediator element = new InteropService.HookMediator(this);
    return this._di.Create<T>(((IEnumerable<object>) param).Append<object>((object) element).ToArray<object>());
  }

  public HookScope CreateScope()
  {
    return new HookScope((IHookMediator) new InteropService.HookMediator(this));
  }
    ...
    public bool IsValid
    {
      get
      {
        if (this._interop.IsDisposing)
          return false;
        HookModule module = this.Module;
        return module != null && module.IsInit;
      }
    }

    public HookMediator(InteropService interop) => this._interop = interop;

    public T Create<T>(params object[] param) where T : HookModule
    {
      return this._interop.CreateModule<T>(param);
    }
