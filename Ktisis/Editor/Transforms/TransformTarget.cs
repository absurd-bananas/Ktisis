...
public class TransformTarget : ITransformTarget, ITransform
{
  ...
  {
    targets = (IEnumerable<SceneEntity>) targets.ToList<SceneEntity>();
  ...
  {
    return this.Primary is ITransform primary ? primary.GetTransform() : (Transform) null;
  }

  public void SetTransform(Transform transform)
  {
    Transform transform1 = this.GetTransform();
    if (transform1 == null)
      return;
  ...
  private void TransformObjects(Transform transform, Transform initial)
  {
    ...
    foreach (SceneEntity sceneEntity in this.Targets.Where<SceneEntity>((Func<SceneEntity, bool>) (tar => tar != null && tar.IsValid && !(tar is BoneNode))))
    {
      if (sceneEntity is ITransform transform2)
      {
        Transform transform1 = transform2.GetTransform();
        if (transform1 != null)
        {
  ...
  {
    Transform delta = new Transform(transform.Position - initial.Position, transform.Rotation / initial.Rotation, transform.Scale / initial.Scale);
    foreach (KeyValuePair<EntityPose, Dictionary<int, List<BoneNode>>> pose in this.PoseMap)
    {
      EntityPose entityPose1;
      Dictionary<int, List<BoneNode>> dictionary1;
      pose.Deconstruct(ref entityPose1, ref dictionary1);
      EntityPose entityPose2 = entityPose1;
      Dictionary<int, List<BoneNode>> dictionary2 = dictionary1;
      ...
      if ((IntPtr) skeleton != IntPtr.Zero && (IntPtr) skeleton->PartialSkeletons != IntPtr.Zero)
      {
        ...
        for (int key = 0; key < (int) partialSkeletonCount; ++key)
        {
          ...
          if (dictionary2.TryGetValue(key, out source))
          {
            ...
            {
              foreach (BoneNode bone in source.Where<BoneNode>((Func<BoneNode, bool>) (bone => bone.IsValid)))
    ...
    BoneNode bone)
  {
    ...
    Transform transform1 = bone.GetTransform();
    if (transform1 == null)
      return;
    ...
    Transform modelTransform1 = HavokPosing.GetModelTransform(hkaPose, boneIndex);
