...
public class TwoJointsSolver(IkModule module) : IDisposable {
	...
	public unsafe bool Solve(hkaPose* poseIn, hkaPose* poseOut, bool frozen = false) {
		...
		byte num1 = 0;
		var num2 = module.SolveTwoJoints(&num1, this.IkSetup, poseIn);
		if (num1 == 0)
			return false;
		((hkaPose)(IntPtr)poseIn).SyncModelSpace();
		if (frozen)
	...
	public unsafe bool SolveGroup(
		...
		var modelTransform = HavokPosing.GetModelTransform(poseOut, (int)group.EndBoneIndex);
		if (modelTransform == null)
			return false;
		var num = group.Mode == TwoJointsMode.Relative ? 1 : 0;
	...
	private unsafe void UpdateModelPose(hkaPose* poseIn, hkaPose* poseOut) {
		var firstJointIdx = this.IkSetup->m_firstJointIdx;
		for (var parent = 1; parent < poseIn->Skeleton->Bones.Length; ++parent) {
			if (parent == firstJointIdx || HavokPosing.IsBoneDescendantOf(poseOut->Skeleton->ParentIndices, firstJointIdx, parent))
	...
	private unsafe void ApplyModelPoseStatic(hkaPose* poseIn, hkaPose* poseOut) {
		...
		var firstJointIdx = this.IkSetup->m_firstJointIdx;
		var endBoneIdx = this.IkSetup->m_endBoneIdx;
		for (var index = 1; index < poseOut->Skeleton->Bones.Length; ++index) {
			if ((index == firstJointIdx ? 1 : HavokPosing.IsBoneDescendantOf(parentIndices, index, firstJointIdx) ? 1 : 0) != 0) {
				if (!HavokPosing.IsBoneDescendantOf(parentIndices, index, endBoneIdx)) {
					...
					var localTransform = HavokPosing.GetLocalTransform(poseIn, index);
					var modelTransform = HavokPosing.GetModelTransform(poseOut, boneIx);
	...
	private unsafe void ApplyModelPoseDynamic(hkaPose* poseIn, hkaPose* poseOut) {
		...
		for (var bone = 1; bone < poseOut->Skeleton->Bones.Length; ++bone) {
			if (bone == firstJointIdx || HavokPosing.IsBoneDescendantOf(parentIndices, bone, firstJointIdx))
