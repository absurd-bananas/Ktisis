...
public class CameraModule : HookModule {
	...
	private unsafe void InitVfHook() {
		...
		if (!this._sigScanner.TryGetStaticAddressFromSig("48 8D 05 ?? ?? ?? ?? C7 83 ?? ?? ?? ?? ?? ?? ?? ?? 48 89 03 0F 57 C0 33 C0 48 C7 83 ?? ?? ?? ?? ?? ?? ?? ??", ref num, 0))
			...
			this.CameraTargetHook = this._interop.HookFromAddress<CameraTargetDelegate>(*(IntPtr*)(num + new IntPtr(17) * sizeof(IntPtr)), new CameraTargetDelegate(this.CameraTargetDetour), (IGameInteropProvider.HookBackend)0);
	}
	...
	private IntPtr CameraTargetDetour(IntPtr a1) {
		var current = this.Manager.Current;
		if (current != null) {
			var orbitTarget = current.OrbitTarget;
	...
	private unsafe CameraRedirect Redirect(bool condition = true) {
		...
		var cameraRedirect = new CameraRedirect(activeCameraIndex);
