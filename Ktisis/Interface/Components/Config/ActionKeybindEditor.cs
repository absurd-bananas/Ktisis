...
public class ActionKeybindEditor
{
  ...
  public void Setup()
  {
    IEnumerable<KeyAction> bindable = this._actions.GetBindable();
    this.Actions.Clear();
    this.Actions.AddRange(bindable);
    this.SetEditing((ActionKeybind) null);
  }
      ...
      using (ImRaii.IEndObject iendObject1 = ImRaii.Child(ImU8String.op_Implicit("##CfgStyleFrame"), Dalamud.Bindings.ImGui.ImGui.GetContentRegionAvail(), false))
      {
        ...
        using (ImRaii.PushStyle((ImGuiStyleVar) 16 /*0x10*/, Vector2.Zero, true))
        {
          using (ImRaii.IEndObject iendObject2 = ImRaii.Table(ImU8String.op_Implicit("##KeyActionTable"), 2, (ImGuiTableFlags) 1921))
          {
            ...
            foreach (KeyAction action in this.Actions)
  ...
  private void DrawAction(KeyAction action)
  {
    ...
    Dalamud.Bindings.ImGui.ImGui.TableNextColumn();
    string str = this._locale.Translate("actions." + action.GetName());
    Dalamud.Bindings.ImGui.ImGui.SetCursorPos(Dalamud.Bindings.ImGui.ImGui.GetCursorPos() + ActionKeybindEditor.CellPadding);
  ...
  private void DrawKeybind(ActionKeybind keybind)
  {
    ...
    using (ImRaii.PushId(imU8String, true))
    {
      ...
      {
        bool flag = this.Editing == keybind;
        ...
        using (ImRaii.PushColor((ImGuiCol) 21, flag ? colorU32 : 0U, true))
        {
          using (ImRaii.PushColor((ImGuiCol) 23, colorU32, true))
          {
            using (ImRaii.PushColor((ImGuiCol) 22, flag ? colorU32 : Dalamud.Bindings.ImGui.ImGui.GetColorU32((ImGuiCol) 8), true))
            {
              ...
              Vector2 vector2_1 = new Vector2(columnWidth, Dalamud.Bindings.ImGui.ImGui.GetFrameHeightWithSpacing()) + ActionKeybindEditor.CellPadding;
              if (flag)
              {
                ImGuiStylePtr style = Dalamud.Bindings.ImGui.ImGui.GetStyle();
                Vector2 vector2_2 = ((ImGuiStylePtr) ref style).ItemSpacing;
                Dalamud.Bindings.ImGui.ImGui.SetCursorPos(Dalamud.Bindings.ImGui.ImGui.GetCursorPos() + ActionKeybindEditor.CellPadding);
                Dalamud.Bindings.ImGui.ImGui.SetNextItemWidth(columnWidth - ActionKeybindEditor.CellPadding.X - vector2_2.X);
                this.EditKeybind(keybind);
                Dalamud.Bindings.ImGui.ImGui.Dummy(ActionKeybindEditor.CellPadding - vector2_2);
              }
              ...
              {
                keybind.Combo = new KeyCombo((VirtualKey) 0, Array.Empty<VirtualKey>());
              }
              else
              {
    ...
    using (ImRaii.PushId(keybind.GetHashCode(), true))
    {
      using (ImRaii.PushColor((ImGuiCol) 49, 0U, true))
      {
        if (this.KeyCombo == null)
          this.KeyCombo = new KeyCombo((VirtualKey) 0, Array.Empty<VirtualKey>());
        List<VirtualKey> list1 = KeyHelpers.GetKeysDown().ToList<VirtualKey>();
        List<VirtualKey> list2 = list1.Except<VirtualKey>((IEnumerable<VirtualKey>) this.KeysHandled).ToList<VirtualKey>();
        ...
        else
        {
          ...
          string shortcutString = (this.KeysHandled.Count > 0 ? this.KeyCombo : keybind.Combo).GetShortcutString();
