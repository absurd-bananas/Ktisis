...
public class BoneCategoryEditor {
	...
	private void BuildCategoryMap() {
		...
		for (var index = -1; index < this.Config.CategoryList.Count; ++index) {
			var parent = index >= 0 ? this.Config.CategoryList[index].Name : null;
			var list = this.Config.CategoryList.Where(cat => cat.ParentCategory == parent).ToList();
			if (list.Count > 0)
	...
	private void DrawCategoryList(string key) {
		...
		if (!this.CategoryMap.TryGetValue(key, out boneCategoryList))
			return;
		boneCategoryList.ForEach(this.DrawListCategory);
	}
	...
	private void DrawCategoryColors(BoneCategory category) {
		...
		if (!(!category.LinkedColors
			? flag | DrawColorEdit(this._locale.Translate("config.categories.editor.group_color"), ref category.GroupColor) | DrawColorEdit(this._locale.Translate("config.categories.editor.bone_color"), ref category.BoneColor)
			: DrawColorEdit(this._locale.Translate("config.categories.editor.group_color"), ref category.GroupColor)) || !this.ColorSub)
			return;
	...
	private void SetColors(BoneCategory parent, BoneCategory child) {
		...
		if (!this.CategoryMap.TryGetValue(child.Name, out boneCategoryList))
			return;
		boneCategoryList.ForEach(cat => this.SetColors(parent, cat));
	}
	...
	private static bool DrawColorEdit(string label, ref uint color) {
		...
		var num = Dalamud.Bindings.ImGui.ImGui.ColorEdit4(ImU8String.op_Implicit(label), ref float4, (ImGuiColorEditFlags)0) ? 1 : 0;
		if (num == 0)
			return num != 0;
