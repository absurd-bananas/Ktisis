...
public class ParamColorSelectPopup
{
  ...
  public void Open(CustomizeIndex index, uint[] colors)
  {
    ...
    // ISSUE: reference to a compiler-generated field
    this.Colors = ((IEnumerable<uint>) colors).Take<uint>(this.IsAlpha ? 96 /*0x60*/ : colors.Length).Select<uint, Vector4>(ParamColorSelectPopup.\u003C\u003EO.\u003C0\u003E__ColorConvertU32ToFloat4 ?? (ParamColorSelectPopup.\u003C\u003EO.\u003C0\u003E__ColorConvertU32ToFloat4 = new Func<uint, Vector4>(Dalamud.Bindings.ImGui.ImGui.ColorConvertU32ToFloat4))).ToArray<Vector4>();
  }
  ...
  public void Draw(ICustomizeEditor editor)
  {
    ...
    using (ImRaii.IEndObject iendObject = ImRaii.Popup(ImU8String.op_Implicit(this.PopupId), (ImGuiWindowFlags) 64 /*0x40*/))
    {
      ...
      {
        this._isOpen = true;
        byte customization = editor.GetCustomization(this.Index);
        if (this.IsAlpha)
        {
  ...
  private void DrawColorInput(ICustomizeEditor editor, byte current)
  {
    ...
    int num = (int) current & (this.IsAlpha ? -129 : (int) byte.MaxValue);
