...
public class NpcSelect
{
  ...
  private NpcSelect.NpcLoadState _npcLoadState;
  ...
  public NpcSelect(NpcService npc, LocaleManager locale)
  {
    ...
    // ISSUE: reference to a compiler-generated field
    this._popup = new PopupList<INpcBase>("##NpcImportPopup", new ListBox<INpcBase>.DrawItemDelegate(this.DrawItem)).WithSearch(NpcSelect.\u003C\u003EO.\u003C0\u003E__MatchQuery ?? (NpcSelect.\u003C\u003EO.\u003C0\u003E__MatchQuery = new PopupList<INpcBase>.SearchPredicate(NpcSelect.MatchQuery)));
  }

  public void Fetch()
  {
    if (this._npcLoadState == NpcSelect.NpcLoadState.Success)
      return;
    this._npc.GetNpcList().ContinueWith((Action<Task<IEnumerable<INpcBase>>>) (task =>
    {
      if (((Task) task).Exception != null)
      {
        ...
        this._npcLoadState = NpcSelect.NpcLoadState.Failed;
      }
      else
      {
        this._npcList.Clear();
        this._npcList.AddRange(task.Result);
        this._npcLoadState = NpcSelect.NpcLoadState.Success;
      }
    }));
  }
  ...
  public void Draw()
  {
    ...
    {
      case NpcSelect.NpcLoadState.Waiting:
        Dalamud.Bindings.ImGui.ImGui.Text(ImU8String.op_Implicit("Loading NPCs..."));
        break;
      case NpcSelect.NpcLoadState.Success:
        this.DrawSelect();
        break;
      case NpcSelect.NpcLoadState.Failed:
  ...
  {
    string str = this.Selected != null ? this.Selected.Name : "Select...";
    ...
    if (Dalamud.Bindings.ImGui.ImGui.IsItemActivated())
      this._popup.Open();
    float itemHeight = Dalamud.Bindings.ImGui.ImGui.GetFontSize() * 2f;
    INpcBase selected;
  ...
  {
    this.Selected = npc;
    OnNpcSelected onSelected = this.OnSelected;
    if (onSelected == null)
      return;
    onSelected(npc);
  }
  ...
  private bool DrawItem(INpcBase npc, bool isFocus)
  {
    ...
    int num = Dalamud.Bindings.ImGui.ImGui.Selectable(ImU8String.op_Implicit("##"), isFocus, (ImGuiSelectableFlags) 0, VectorExtensions.WithY(Dalamud.Bindings.ImGui.ImGui.GetContentRegionAvail(), fontSize * 2f)) ? 1 : 0;
    ...
    if (modelId == (ushort) 0)
    {
      ...
      if (customize.HasValue && customize.Value.Tribe != (Tribe) 0)
      {
        string str1 = customize.Value.Gender == Gender.Masculine ? "♂" : "♀";
        string str2 = this._locale.Translate($"{customize.Value.Tribe}");
