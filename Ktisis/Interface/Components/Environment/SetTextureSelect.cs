...
public class SetTextureSelect {
	...
	private readonly static Vector2 ButtonSize = new Vector2(48f, 48f);
	private readonly static Vector2 OptionSize = new Vector2(64f, 64f);
	...
	public bool Draw(string name, ref uint value, ResolvePathHandler resolve) {
		...
		using (ImRaii.PushId(imU8String, true)) {
			...
			if (this.DrawButton(value, fromGame, ButtonSize))
				this.OpenPopup(name, resolve);
			var flag2 = flag1 | this.DrawPopup(name, ref value);
	...
	private bool DrawPopup(string name, ref uint value) {
		...
		Dalamud.Bindings.ImGui.ImGui.SetNextWindowSizeConstraints(Vector2.Zero, new Vector2((float)(((double)OptionSize.X + (double)((ImGuiStylePtr) ref style).ItemSpacing.X) * 6.0) +((ImGuiStylePtr) ref style)
			.ItemInnerSpacing.X + ((ImGuiStylePtr) ref style).ScrollbarSize, (float)(((double)OptionSize.Y + (double)((ImGuiStylePtr) ref style).ItemSpacing.Y) *4.0) +((ImGuiStylePtr) ref style).WindowPadding.Y));
		ImU8String imU8String;
		// ISSUE: explicit constructor call
		((ImU8String) ref imU8String).\u002Ector(6, 1);
		((ImU8String) ref imU8String).AppendFormatted<string>(name);
	...
	private class OptionsPopupResource : IDisposable {
		...
		public void Load(ITextureProvider tex, ResolvePathHandler resolve) {
			this.LoadAsync(Enumerable.Range(0, 1000).Select(i => (uint)i).Select(i => {
			...
			}).Where(opt => opt != null), this.Source.Token).ContinueWith(task => {
		...
		private async Task LoadAsync(
			...
			t.Start();
			foreach (Option[] collection in Enumerable.Chunk<Option>(values, 5)) {
