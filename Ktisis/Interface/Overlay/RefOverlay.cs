...
public class RefOverlay {
	...
	public void DrawInstance(ReferenceImage image) {
		...
		if (!visible || !this._tex.GetFromFile(image.Data.FilePath).TryGetWrap(ref idalamudTextureWrap, ref exception))
			return;
		var drawReferenceTitle = this._cfg.File.Overlay.DrawReferenceTitle;
		Dalamud.Bindings.ImGui.ImGui.SetNextWindowSize(idalamudTextureWrap.Size, (ImGuiCond)4);
		HandleImageAspectRatio(idalamudTextureWrap.Size, drawReferenceTitle);
	...
	private unsafe static void HandleImageAspectRatio(Vector2 size, bool title) {
		...
		_data.Ratio = num;
		_data.Height = title ? Dalamud.Bindings.ImGui.ImGui.GetFrameHeight() : 0.0f;
		fixed (CallbackData* callbackDataPtr1 = &_data) {
			...
			var callbackDataPtr2 = callbackDataPtr1;
	...
	private unsafe static void SetSizeCallback(ImGuiSizeCallbackData* data) {
		if ((IntPtr)data == IntPtr.Zero)
			return;
		var userData = (CallbackData*)data->UserData;
