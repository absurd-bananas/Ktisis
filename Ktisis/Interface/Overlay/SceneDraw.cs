...
public class SceneDraw
{
  ...
  {
    ISelectableFrame frame = this._select.BeginFrame();
  ...
  private void DrawEntities(ISelectableFrame frame, IEnumerable<SceneEntity> entities)
  {
    ...
    foreach (SceneEntity entity in entities)
    {
      ...
      if (entity is ActorEntity actor)
      {
        ...
        if ((double) opacityMultiplier == 0.0)
          continue;
      }
    ...
    float opacityMultiplier)
  {
    ...
    List<BoneNode> list = !this.Config.DrawLines || !this.Config.ColorSelectedBoneParentLine && !this.Config.ColorSelectedBoneDescendantLine || this.Config.DrawLinesGizmo && Ktisis.ImGuizmo.Gizmo.IsUsing ? (List<BoneNode>) null : pose.GetAllBones().Where<BoneNode>((Func<BoneNode, bool>) (x => x.IsSelected)).ToList<BoneNode>();
    HashSet<BoneNode> descendantSet = (HashSet<BoneNode>) null;
    if (list != null && this.Config.ColorSelectedBoneDescendantLine)
    {
      descendantSet = new HashSet<BoneNode>();
      foreach (BoneNode node in list)
      {
    ...
    for (int partialIx = 0; partialIx < (int) partialSkeletonCount; ++partialIx)
    {
      ...
      if ((IntPtr) havokPose != IntPtr.Zero && (IntPtr) havokPose->Skeleton != IntPtr.Zero)
      {
        ...
        for (int boneIx1 = 0; boneIx1 < length; ++boneIx1)
        {
          BoneNode boneFromMap1 = pose.GetBoneFromMap(partialIx, boneIx1);
          // ISSUE: explicit non-virtual call
          if ((boneFromMap1 != null ? (!__nonvirtual (boneFromMap1.Visible) ? 1 : 0) : 1) == 0)
          {
            Transform transform1 = boneFromMap1.CalcTransformWorld();
            if (transform1 != null)
            {
              frame.AddItem((SceneEntity) boneFromMap1, transform1.Position, opacityMultiplier);
              if (this.Config.DrawLines && (this.Config.DrawLinesGizmo || !Ktisis.ImGuizmo.Gizmo.IsUsing))
              {
                for (int boneIx2 = boneIx1; boneIx2 < length; ++boneIx2)
                {
                  if ((int) skeleton2->ParentIndices[boneIx2] == boneIx1)
                  {
                    BoneNode boneFromMap2 = pose.GetBoneFromMap(partialIx, boneIx2);
                    // ISSUE: explicit non-virtual call
                    if ((boneFromMap2 != null ? (!__nonvirtual (boneFromMap2.Visible) ? 1 : 0) : 1) == 0)
                    {
                      Transform transform2 = boneFromMap2.CalcTransformWorld();
                      if (transform2 != null)
                      {
                        EntityDisplay entityDisplay = this._ctx.Config.GetEntityDisplay((SceneEntity) boneFromMap1);
  ...
  {
    if ((double) this.Config.ActiveActorOpacityMultiplier == (double) this.Config.InactiveActorOpacityMultiplier)
    ...
    {
      actor.Recurse();
      if (actor.IsSelected || actor.Recurse().Any<SceneEntity>((Func<SceneEntity, bool>) (x => x.IsSelected)))
        return this.Config.ActiveActorOpacityMultiplier;
    }
    ...
    float opacityMultiplier)
  {
    ...
    if (!CameraService.WorldToScreen(camera, fromPos, out screenPos1) || !CameraService.WorldToScreen(camera, toPos, out screenPos2))
      return;
    float num = Ktisis.ImGuizmo.Gizmo.IsUsing ? this.Config.LineOpacityUsing : this.Config.LineOpacity;
