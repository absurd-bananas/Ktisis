...
public class ActorCollectionPopup : KtisisPopup
{
    ...
    : base("##ActorCollectionPopup")
  {
    ...
    this._list = new ListBox<KeyValuePair<Guid, string>>("##CollectionList", new ListBox<KeyValuePair<Guid, string>>.DrawItemDelegate(this.DrawItem));
  }
  ...
  protected override void OnDraw()
  {
    ...
    else
    {
      ...
      if (!this._list.Draw(this._ipc.GetCollections().ToList<KeyValuePair<Guid, string>>(), out selected) || !this._ipc.SetCollectionForObject(this._entity.Actor, selected.Key))
        return;
      this._entity.Redraw();
    }
  }
  ...
  {
    return Dalamud.Bindings.ImGui.ImGui.Selectable(ImU8String.op_Implicit(item.Value), item.Key == this._current.Id, (ImGuiSelectableFlags) 0, new Vector2());
  }
}
