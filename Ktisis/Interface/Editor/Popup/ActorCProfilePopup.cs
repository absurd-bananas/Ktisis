...
public class ActorCProfilePopup : KtisisPopup
{
    ...
    : base("##ActorCProfilePopup")
  {
    ...
    this._list = new ListBox<(Guid, string, string, List<(string, ushort, byte, ushort)>, int, bool)>("##CProfileList", new ListBox<(Guid, string, string, List<(string, ushort, byte, ushort)>, int, bool)>.DrawItemDelegate(this.DrawItem));
  }
  ...
  protected override void OnDraw()
  {
    ...
    else
    {
      ...
      Guid? id = this._ipc.GetActiveProfileId(this._entity.Actor.ObjectIndex).Id;
      if (id.HasValue)
      {
        foreach ((Guid UniqueId, string Name, string VirtualPath, List<(string Name, ushort WorldId, byte CharacterType, ushort CharacterSubType)> Characters, int Priority, bool IsEnabled) profile in this._profiles)
        {
          Guid uniqueId = profile.UniqueId;
          Guid? nullable = id;
  ...
  private bool DrawItem(
    ...
    bool _)
  {
    return Dalamud.Bindings.ImGui.ImGui.Selectable(ImU8String.op_Implicit(item.Name), item.UniqueId == this._current.Id, (ImGuiSelectableFlags) 0, new Vector2());
  }
}
