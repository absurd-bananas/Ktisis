...
public class ObjectWindow : KtisisWindow
{
  ...
  {
    SceneEntity entity = this._ctx.Selection.GetFirstSelected() ?? target?.Primary;
    if (entity == null)
      return;
  ...
  private bool DrawTransform(ref Ktisis.Common.Utility.Transform transform, out bool isEnded, bool disabled)
  {
    ...
    bool flag2 = this._table.Draw(transform, out transOut, TransformTableFlags.Default | TransformTableFlags.UseAvailable);
    if (flag2)
      transform = transOut;
  ...
  private unsafe bool DrawGizmo(ref Ktisis.Common.Utility.Transform transform, float width, bool disabled)
  {
    ...
    Gizmo2D gizmo = this._gizmo;
    Vector3 cameraPos = vector3;
    Vector3 translation = matrix.Translation;
    double fov = (double) num1;
    double x = (double) rectSize.X;
    ImGuiStylePtr style = Dalamud.Bindings.ImGui.ImGui.GetStyle();
    double num2 = (double) ((ImGuiStylePtr) ref style).WindowPadding.X * 2.0;
    double num3 = x - num2;
    double y = (double) rectSize.Y;
    style = Dalamud.Bindings.ImGui.ImGui.GetStyle();
    double num4 = (double) ((ImGuiStylePtr) ref style).WindowPadding.Y * 2.0;
    double num5 = y - num4;
    double aspect = num3 / num5;
    gizmo.SetLookAt(cameraPos, translation, (float) fov, (float) aspect);
    int num6 = this._gizmo.Manipulate(ref matrix, out Matrix4x4 _) ? 1 : 0;
    this._gizmo.End();
    if (num6 == 0)
      return num6 != 0;
