...
public class LightWindow : EntityEditWindow<LightEntity>
{
  ...
  {
    this.UpdateTarget();
    ISelectManager selection = this.Context.Selection;
    if (selection.Count == 1 && selection.GetSelected().First<SceneEntity>() is LightEntity target1)
      this.SetTarget(target1);
    LightEntity target2 = this.Target;
    ...
    {
      this.DrawTab("Light", new Action<LightEntity>(this.DrawLightTab), target2);
      this.DrawTab("Shadows", new Action<LightEntity>(this.DrawShadowsTab), target2);
  ...
  private void DrawImportExport(LightEntity entity1)
  {
    ...
    {
      IEnumerable<ActorEntity> source = this._ctx.Scene.Children.Where<SceneEntity>((Func<SceneEntity, bool>) (entity2 => entity2 is ActorEntity)).Cast<ActorEntity>();
    ...
    if (flag)
      this._ctx.Interface.OpenLightExportNoActorSelected((Action) (() => this._ctx.Interface.OpenLightExport(entity1)), this._ctx.Config);
    else
  ...
  {
    SceneLight* sceneLightPtr = entity.GetObject();
    ...
    Dalamud.Bindings.ImGui.ImGui.Spacing();
    string str1 = this._locale.Translate($"lightType.{renderLight->LightType}");
    ...
    Dalamud.Bindings.ImGui.ImGui.Spacing();
    string str2 = this._locale.Translate($"lightFalloff.{renderLight->FalloffType}");
  ...
  {
    bool flag1 = light->Flags.HasFlag((Enum) flag);
