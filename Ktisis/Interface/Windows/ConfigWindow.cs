...
public class ConfigWindow : KtisisWindow {
	...
	public virtual void Draw() {
		using (ImRaii.IEndObject iendObject = ImRaii.TabBar(ImU8String.op_Implicit("##ConfigTabs"))) {
			...
			DrawTab("Output", this.DrawOutputTab);
		}
	}
	...
	private void DrawCategoriesTab() {
		var num1 = 0 | (Dalamud.Bindings.ImGui.ImGui.Checkbox(ImU8String.op_Implicit(this.Locale.Translate("config.categories.allow_nsfw")), ref this.Config.Categories.ShowNsfwBones) ? 1 : 0);
		this.DrawHint("config.categories.hint_nsfw");
		var num2 = Dalamud.Bindings.ImGui.ImGui.Checkbox(ImU8String.op_Implicit(this.Locale.Translate("config.categories.show_all_viera_ears")), ref this.Config.Categories.ShowAllVieraEars) ? 1 : 0;
		var num3 = num1 | num2;
	...
	private void DrawAutoSaveTab() {
		var autoSave = this.Config.AutoSave;
		...
		Dalamud.Bindings.ImGui.ImGui.InputText(ImU8String.op_Implicit(this.Locale.Translate("config.autosave.dir")), ref autoSave.FolderFormat, 256 /*0x0100*/, (ImGuiInputTextFlags)0,
			(Dalamud.Bindings.ImGui.ImGui.ImGuiInputTextCallbackDelegate)null);
	...
	private void DrawAutoSaveFormatting() {
		...
		using (ImRaii.IEndObject iendObject = ImRaii.Table(imU8String, 2, (ImGuiTableFlags)2107264)) {
			...
			foreach (var replacement in this._format.GetReplacements()) {
				string str1;
				string str2;
	...
	private void DrawOutputTab() {
		...
		var index1 = 0;
		Size resolution;
		for (var index2 = 0; index2 < pyon.Resolutions.Count; ++index2) {
		...
		var array = pyon.Resolutions.Select((Func<Size, string>)(r => $"{r.Width} x {r.Height}")).ToArray();
