...
public class BoneNodeGroup : SkeletonGroup, IAttachTarget
{
  ...
  {
    BoneCategory category = this.Category;
  ...
  {
    BoneNode boneNode = Enumerable.MinBy<BoneNode, int>(this.GetIndividualBones().Where<BoneNode>((Func<BoneNode, bool>) (bone => bone.Info.PartialIndex == 0)), (Func<BoneNode, int>) (bone => bone.Info.BoneIndex));
