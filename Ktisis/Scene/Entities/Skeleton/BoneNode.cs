...
public class BoneNode : SkeletonNode, ITransform, IVisibility, IAttachTarget {
	...
	public unsafe FFXIVClientStructs.FFXIV.Client.Graphics.Render.Skeleton* GetSkeleton() => this.Pose.GetSkeleton();
	...
	public Transform? CalcTransformWorld() {
		...
		return !nullable.HasValue ? null : new Transform(nullable.Value);
	}

	public Transform? GetTransformModel() {
		Matrix4x4? matrixModel = this.GetMatrixModel();
		return !matrixModel.HasValue ? null : new Transform(matrixModel.Value);
	}
	...
	public unsafe bool IsBoneDescendantOf(BoneNode node) {
		...
		var partialIndex1 = this.Info.PartialIndex;
		var partialIndex2 = node.Info.PartialIndex;
		hkaPose* havokPose;
		int bone;
		int boneIndex;
	...
	public bool IsVieraEarBone() => this.Info.Name.Length >= 7 && this.Info.Name.StartsWith("j_zer") && this.Info.Name[6] == '_';
}
