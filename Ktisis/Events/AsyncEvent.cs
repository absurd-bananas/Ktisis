...
public class AsyncEvent<T> : EventBase<T> where T : Delegate {
	public Task InvokeAsync() => this.RunInvoke(sub => ((Func<Task>)sub)());

	public Task InvokeAsync<T1>(T1 a1) {
		return this.RunInvoke(sub => ((Func<T1, Task>)sub)(a1));
	}

	public Task InvokeAsync<T1, T2>(T1 a1, T2 a2) {
		return this.RunInvoke(sub => ((Func<T1, T2, Task>)sub)(a1, a2));
	}

	public Task InvokeAsync<T1, T2, T3>(T1 a1, T2 a2, T3 a3) {
		return this.RunInvoke(sub => ((Func<T1, T2, T3, Task>)sub)(a1, a2, a3));
	}

	private async Task RunInvoke(Func<object, Task> selector) {
		var asyncEvent = this;
		await Task.WhenAll(asyncEvent._subscribers.Select(selector)).ContinueWith(asyncEvent.LogException);
	}
